...
import { FaTag, FaCalendarAlt, FaUsers, FaBox, FaPercentage, FaEye, FaCheck, FaTimes } from 'react-icons/fa';
...
const FlashDealCampaigns: React.FC = () => {
  ...
  const fetchCampaigns = async () => {
    try {
      ...
      const params: any = { campaign_type: 'Flash' };
      if (filter !== 'all') {
    ...
    { 
      ...
      render: (item: Campaign) => (
        <div className="text-xs">
          <div>Min Discount: {item.minimum_discount}%</div>
  ...
  const participationColumns = [
    ...
    { 
      ...
      label: 'Requirements',
      render: (item: VendorParticipation) => (
        item.meets_minimum_requirements ? (
          <span className={`px-2 py-1 rounded-full text-xs ${
            item.meets_minimum_requirements.meets 
              ? 'bg-green-100 text-green-800' 
              : 'bg-red-100 text-red-800'
          }`}>
            {item.meets_minimum_requirements.meets ? '✓ Meets' : '✗ Not Met'}
          </span>
        ) : '-'
      )
    },
      ...
      render: (item: CampaignProduct) => (
        item.meets_minimum_discount ? (
          <span className="text-green-600 flex items-center gap-1">
            <FaCheck /> Meets {selectedCampaign?.minimum_discount}%
          </span>
        ) : (
          <span className="text-red-600 flex items-center gap-1">
            <FaTimes /> Below {selectedCampaign?.minimum_discount}%
          </span>
        )
      )
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-900">Flash Deal Campaigns</h1>
      ...
      <DataTable
        ...
        emptyMessage="No Flash Deal campaigns found"
      />
...
export default FlashDealCampaigns;