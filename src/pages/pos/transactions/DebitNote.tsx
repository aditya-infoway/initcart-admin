...
interface NoteRow {
  id: number;
  noteId: string; // DN-001 / CN-001 auto
  refInvoice: string;
  date: string;
  type: NoteType;
  amount: number;
...
export const DebitNote = () => {
  ...
  const handleSave = () => {
    if (!form.refInvoice || !form.date || !form.amount)
      return Swal.fire("Required", "Ref Invoice, Date, Amount.", "warning");
    if (editing)
      setRows((list) =>
      ...
      setRows((list) => [
        { id: next, noteId: `${String(next).padStart(3, "0")}`, ...form },
        ...list,
      ]);
    }
    setShowForm(false);
        ...
        <div className="fixed inset-0 bg-[#0000007d] z-50 flex items-center justify-center px-3">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6 relative">
            <h2 className="text-xl font-bold mb-4">
              {editing ? `Edit ${"DebitNote"}` : `Add ${"DebitNote"}`}
            </h2>
