// src/pages/admin/TechIndustryAdsManager.tsx
...
const TechIndustryAdsManager: React.FC = () => {
  ...
  const loadBigAd = async () => {
    try {
      const res = await apiClient.get("tech-industry-big-ad/");
      if (!res.data) return;
  ...
  const loadSmallAds = async () => {
    try {
      const res = await apiClient.get("tech-industry-small-ad/");
 ...
 const handleImage = (
  ...
  const handleBigSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post("tech-industry-big-ad/", fd, {
  ...
  const handleSmallSubmit = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < smallAds.length; i++) {
      ...
      await apiClient.post("tech-industry-small-ad/", fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });
    }
  ...
  const inputStyle =
    "w-full border border-slate-200 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-200 p-3 rounded-xl outline-none transition";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 to-slate-200 p-6">
      ...
      >
        <div className="text-center space-y-2">
          <h1 className="text-4xl font-bold tracking-tight">Tech Industry Ads Manager</h1>
          <p className="text-slate-500">Manage homepage banner & small ads for Tech Industry</p>
        </div>
        ...
        {mode === "big" && (
          ...
          >
            ...
            <button className="w-full bg-cyan-600 hover:bg-cyan-700 text-white py-3 rounded-xl font-medium shadow transition">
              Save Big Ad
            </button>
          </motion.form>
        )}
...
export default TechIndustryAdsManager;