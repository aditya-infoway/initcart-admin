...
const TravelAgencyServiceAdsManager: React.FC = () => {
  ...
  const loadBigAd = async () => {
    try {
      const res = await apiClient.get("travelagency-big-ad/");
      if (!res.data) return;
  ...
  const loadSmallAds = async () => {
    try {
      const res = await apiClient.get("travelagency-small-ad/");
 ...
 const handleImage = (
  ...
  const handleBigSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post("travelagency-big-ad/", fd, {
  ...
  const handleSmallSubmit = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < smallAds.length; i++) {
      ...
      await apiClient.post("travelagency-small-ad/", fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 to-slate-200 p-6">
      ...
      >
        <div className="text-center space-y-2">
          <h1 className="text-4xl font-bold tracking-tight">Travel Agency Ads Manager</h1>
...
export default TravelAgencyServiceAdsManager;
