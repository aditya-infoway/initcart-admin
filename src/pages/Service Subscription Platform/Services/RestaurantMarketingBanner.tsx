// src/pages/admin/ads/RestaurantAdsManager.tsx
...
const RestaurantAdsManager: React.FC = () => {
  ...
  const loadBigAd = async () => {
    try {
      const res = await apiClient.get("restaurant-big-ad/");
      if (!res.data) return;
  ...
  const loadSmallAds = async () => {
    try {
      const res = await apiClient.get("restaurant-small-ad/");
  ...
  const handleBigSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post("restaurant-big-ad/", fd, {
  ...
  const handleSmallSubmit = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < smallAds.length; i++) {
      ...
      await apiClient.post("restaurant-small-ad/", fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });
    }
  ...
  const inputStyle =
    "w-full border border-slate-200 focus:border-orange-500 focus:ring-2 focus:ring-orange-200 p-3 rounded-xl outline-none transition";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 to-slate-200 p-6">
      ...
      >
        <div className="text-center space-y-2">
          <h1 className="text-4xl font-bold tracking-tight text-gray-800">Restaurant Ads Manager</h1>
          <p className="text-slate-500">Manage homepage banner & small ads for Restaurant Services</p>
        </div>
        ...
        {mode === "big" && (
          ...
          >
            ...
            <button className="w-full bg-orange-600 hover:bg-orange-700 text-white py-3 rounded-xl font-medium shadow transition">
              Save Big Ad
            </button>
          </motion.form>
        )}
        ...
        {mode === "small" && (
          ...
          >
            ...
            <button className="w-full bg-red-600 hover:bg-red-700 text-white py-3 rounded-xl font-medium shadow transition">
              Save Small Ads
            </button>
          </motion.form>
        )}
...
export default RestaurantAdsManager;