...
const RealEstateServiceAdsManager: React.FC = () => {
  ...
  const loadBigAd = async () => {
    try {
      const res = await apiClient.get("realestate-big-ad/");
      if (!res.data) return;
  ...
  const loadSmallAds = async () => {
    try {
      const res = await apiClient.get("realestate-small-ad/");
 ...
 const handleImage = (
  ...
  const handleBigSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post("realestate-big-ad/", fd, {
  ...
  const handleSmallSubmit = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < smallAds.length; i++) {
      ...
      await apiClient.post("realestate-small-ad/", fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 to-slate-200 p-6">
      ...
      >
        <div className="text-center space-y-2">
          <h1 className="text-4xl font-bold tracking-tight">Real Estate Ads Manager</h1>
...
export default RealEstateServiceAdsManager;
